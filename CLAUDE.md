...
- `/ebook-review` の校正者SubAgentが自動で検出するが、校閲者も二重チェックする
...
3. **中間報告で止まらない**: SubAgentの結果を受け取ったら、報告のみで止まらず即座に次の工程のSubAgentを起動する
4. **エラー時のみ停止**: 致命的なエラー（ファイルが存在しない、権限エラー等）が発生した場合のみユーザーに報告して停止する

### SubAgent起動の連続性ルール

```
SubAgent完了 → 結果確認 → 即座に次のSubAgent起動（同一メッセージ内）
```

- SubAgentの結果を受け取ったら、**同じレスポンス内で**次の工程のSubAgentを起動すること
- 「結果を報告して終了」ではなく「結果を確認し、次のアクションを実行して終了」を徹底する
- 並列実行可能なSubAgentは必ず1つのメッセージ内で同時に起動する
...
| 禁止パターン | 正しい行動 |
|-------------|-----------|
| SubAgent完了後「次に進めます」と報告して停止 | 報告せず即座に次のSubAgentを起動 |
| 「アウトラインが完成しました。執筆に進みますか？」 | アウトライン完成 → 即座にライターSubAgentを起動 |
