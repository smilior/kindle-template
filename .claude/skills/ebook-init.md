...
| 環境変数 | 用途 | デフォルト適用先 |
|---------|------|----------------|
| `AUTHOR_NAME` | 著者名（ペンネーム） | Phase 1: 著者名の初期値 |
| `DEFAULT_TONE` | 文体 | Phase 1: 文体の初期値 |
| `WRITING_STYLE` | 文章の雰囲気・トーン | Phase 2: 戦略設計の入力、執筆時の文体指針 |
| `AUTHOR_BACKGROUND` | 著者バックグラウンド | Phase 2: 戦略設計の入力 |

これらの値が `.env` に設定済みの場合、Phase 1 の対応する質問をスキップまたは初期値として使用する。

## 手順

### `/ebook-idea` からのデータ引き継ぎ

`/ebook-idea` から呼ばれた場合、テーマ調査で得られた構造化データ（タイトル案・BDF・ニッチ公式・キーワード・差別化・構成タイプ等）が事前確定済み。

この場合、Phase 1 では以下の項目のみユーザーに確認する：
- 著者名
- 文体（です・ます / だ・である）
- 目標章数（デフォルト: 8章）
- 目標文字数（デフォルト: 20,000文字）

Phase 2（戦略設計）は `/ebook-idea` で完了済みのためスキップし、Phase 3 に進む。
