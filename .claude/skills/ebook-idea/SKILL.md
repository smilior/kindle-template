...
| 環境変数 | 用途 | Phase |
|---------|------|-------|
| `AUTHOR_BACKGROUND` | 著者バックグラウンド（著者適合度スコアリングに使用） | Phase 1, 2, 3 |
| `WRITING_STYLE` | 文章の雰囲気・トーン（テーマとの相性判断に使用） | Phase 3 |
| `EXCLUDED_TOPICS` | 除外テーマ（カンマ区切り） | Phase 1, 2 |

これらの値が `.env` に設定済みの場合、Phase 1 の対応する質問をスキップまたは初期値として使用する。
...
- 著者名（`.env` の `AUTHOR_NAME` があればデフォルト値として提示）
- 文体（`.env` の `DEFAULT_TONE` があればデフォルト値として提示）
- 目標章数（デフォルト: 8章）
